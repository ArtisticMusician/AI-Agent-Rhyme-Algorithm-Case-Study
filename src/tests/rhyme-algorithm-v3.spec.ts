...
// src/tests/rhyme-algorithm-v3.spec.ts

import { isPerfectRhymeV3 } from '../algorithms/rhyme-algorithm-v3';
import testPerfectRhymesData from '../data/TestPerfect2.json'; // Use TestPerfect2.json
import dictionary from '../data/dictionary.json';

describe('Perfect Rhyme Algorithm V3 Evaluation', () => {
  const targetWord = 'test';
  // TestPerfect2.json is now a flat object where keys are the words
  const allPerfectRhymes: string[] = Object.keys(testPerfectRhymesData).map(word => word.toLowerCase());
  ...
  it('should evaluate the F1-Score for Algorithm V3', () => {
    let truePositives = 0;
    ...
    for (const word in dictionary) {
      ...
      if (isPerfectRhymeV3(targetWord, word)) {
    ...
    console.log(`\n--- Algorithm V3 Results for "${targetWord}" ---`);
